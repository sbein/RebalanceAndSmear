...
var_MhtLessNonJetsJet2p4 = np.zeros(1,dtype=float)
...
var_MhtLessNonJetsJet5p0 = np.zeros(1,dtype=float)
...
var_NElectrons = np.zeros(1,dtype=int)
var_NMuons = np.zeros(1,dtype=int)
var_NPhotons = np.zeros(1,dtype=int)
var_ElPt = np.zeros(1,dtype=float)
var_ElEta = np.zeros(1,dtype=float)
var_MuPt = np.zeros(1,dtype=float)
var_MuEta = np.zeros(1,dtype=float)
var_PhoPt = np.zeros(1,dtype=float)
var_PhoEta = np.zeros(1,dtype=float)
...
if isdata:
    ...
    var_PassesTrigSingleEl45 = np.zeros(1,dtype=int)
...
tEvent.Branch('MhtLessNonJetsJet2p4', var_MhtLessNonJetsJet2p4,'MhtLessNonJetsJet2p4/D')
tEvent.Branch('MhtLessNonJetsJet5p0', var_MhtLessNonJetsJet5p0,'MhtLessNonJetsJet5p0/D')
...
tEvent.Branch('NElectrons', var_NLeptons,'NLeptons/I')
tEvent.Branch('NMuons', var_NLeptons,'NLeptons/I')
tEvent.Branch('NPhotons', var_NPhotons,'NPhotons/I')
tEvent.Branch('ElPt', var_ElPt,'ElPt/D')
tEvent.Branch('ElEta', var_ElEta,'ElEta/D')
tEvent.Branch('MuPt', var_MuPt,'MuPt/D')
tEvent.Branch('MuEta', var_MuEta,'MuEta/D')
tEvent.Branch('PhoPt', var_PhoPt,'PhoPt/D')
tEvent.Branch('PhoEta', var_PhoEta,'PhoEta/D')
...
if isdata:
    ...
    tEvent.Branch('PassesTrigSingleEl45', var_PassesTrigSingleEl45,'PassesTrigSingleEl45/I')        
...
triggerIndecesV15['SingleEl45'] = [41]#[21, 23, 28, 35, 40, 41]
...
def SelectCrSel(c):
    ...
    if not ((c.MHT>120 or len(c.Muons)>0) and c.Electrons[0].Pt()>30 and c.Electrons_passIso[0] and c.Electrons_tightID[0] and c.HT>300): return False #this
    else: return True
def SelectCrSpho(c):
...
def SelectLooseBaseline(c):
    if not(c.NElectrons==0 and c.NMuons==0 and c.isoElectronTracksclean==0 and c.isoPionTracksclean==0 and c.isoMuonTracksclean==0 and c.MHT>200): return False
    return True     
def AlwaysTrue(c):
    return True
...
for ientry in range(nentries):
    ...
    if not RegionSelection(c): continue

    var_Met[0] = c.MET

    el = TLorentzVector()
    try: el = c.Electrons[0]
    except: el.SetPxPyPzE(0,0,1,0)
    mu = TLorentzVector()
    try: mu = c.Muons[0]
    except: mu.SetPxPyPzE(0,0,1,0)
    pho = TLorentzVector()
    try: pho = c.Photons[0]
    except: pho.SetPxPyPzE(0,0,1,0)                
    ...
    var_MuEta[0] = mu.Eta()     
    var_PhoPt[0] = pho.Pt()
    var_PhoEta[0] = pho.Eta()        
    
    
    mhtvec_jet2p4 = TLorentzVector()   
    jets_jet2p4 = []
    passDphi_jet2p4 = False    
    ht_jet2p4 = 0        

    mhtvec_jet5p0 = TLorentzVector()
    jets_jet5p0 = []
    passDphi_jet5p0 = False   
    ht_jet5p0 = 0 
    ...
    for ijet, jet in enumerate(c.Jets):
        ...
        if not abs(jet.DeltaPhi(el))>0.4 and abs(jet.DeltaPhi(mu))>0.4 and abs(jet.DeltaPhi(pho))>0.4: continue
    ...
    mhtvecLessNonJets_jet2p4=mhtvec_jet2p4-(el+mu+pho)
    mhtvecLessNonJets_jet5p0=mhtvec_jet5p0-(el+mu+pho)    
    var_MhtLessNonJetsJet2p4[0] = mhtvecLessNonJets_jet2p4.Pt()    
    var_MhtLessNonJetsJet5p0[0] = mhtvecLessNonJets_jet5p0.Pt()
        
    mindphi_jet2p4 = 9999
    for jet in jets_jet2p4: mindphi_jet2p4 = min(mindphi_jet2p4,abs(jet.DeltaPhi(mhtvec_jet2p4)))
    var_MinDeltaPhiJetsMhtJet2p4[0] = mindphi_jet2p4        
    mindphi_jet5p0 = 999        
    for jet in jets_jet5p0: mindphi_jet5p0 = min(mindphi_jet5p0,abs(jet.DeltaPhi(mhtvec_jet5p0)))
    ...
    if len(jets_jet2p4)>0: var_PassesDeltaPhiJet2p4[0] = abs(jets_jet2p4[0].DeltaPhi(mhtvec_jet2p4))>0.5
    if len(jets_jet2p4)>1: var_PassesDeltaPhiJet2p4[0]*= (abs(jets_jet2p4[1].DeltaPhi(mhtvec_jet2p4))>0.5)
    if len(jets_jet2p4)>2: var_PassesDeltaPhiJet2p4[0]*= (abs(jets_jet2p4[2].DeltaPhi(mhtvec_jet2p4))>0.3)
    if len(jets_jet2p4)>3: var_PassesDeltaPhiJet2p4[0]*= (abs(jets_jet2p4[3].DeltaPhi(mhtvec_jet2p4))>0.3)
    ...
    try: var_CrossSection[0] = c.CrossSection
    except: pass
